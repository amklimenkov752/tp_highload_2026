...
Для API выберем четыре независимых дата-центра, работающих в режиме **active-active**: Москва, Санкт-Петербург, Новосибирск и Владивосток. В Екатеринбурге разместим отдельный **Anycast CDN PoP**: он кеширует и отдаёт изображения, но не исполняет транзакционные API-запросы.
...
- Зарубежная аудитория составляет 5,3%, причём ни одна страна не даёт более 1% трафика [[2]](#source2). На текущем масштабе отдельный зарубежный ДЦ не окупается: изображения для этой аудитории доставляет Anycast CDN, а API направляется в ближайший доступный российский ДЦ.
...
В штатном режиме GeoDNS поддерживает целевое распределение **20% / 20% / 30% / 30%**: Москва — 20%, Санкт-Петербург — 20%, Новосибирск — 30%, Владивосток — 30%. Так восточный трафик не концентрируется в Новосибирске. Географическая близость учитывается при выборе ответа DNS, а веса корректируются по доступной ёмкости.
...
```mermaid
flowchart LR
    U[Пользователь] -->|api.marketplace.example| DNS[GeoDNS]
    DNS --> M[Москва: L7 balancer]
    DNS --> S[Санкт-Петербург: L7 balancer]
    DNS --> N[Новосибирск: L7 balancer]
    DNS --> V[Владивосток: L7 balancer]
    M & S & N & V --> API[Stateless API-домены]
    U -->|img.marketplace.example| A[Anycast CDN\n203.0.113.0/24 — пример]
    A --> C[CDN edge: Москва, СПб, Екатеринбург, Новосибирск, Владивосток]
    C --> O[Кеш или origin]
```

- Для `api.marketplace.example` используется GeoDNS с TTL 30 секунд. Он выдаёт адрес L7-балансировщика одного из четырёх ДЦ. API stateless: после повторного DNS-разрешения запрос может безопасно попасть на другую живую площадку.
- Для `img.marketplace.example` используется отдельный Anycast IPv4-префикс `/24`. Его анонсируют CDN edge в Москве, Санкт-Петербурге, Екатеринбурге, Новосибирске и Владивостоке; пользователь попадает на ближайшую по маршрутизации доступную площадку. `203.0.113.0/24` на схеме — документационный пример, а не реальная сеть сервиса.
- Global traffic manager проверяет `/health` каждого ДЦ каждые 5 секунд. После двух последовательных неуспешных проверок он обнуляет DNS-вес ДЦ и распределяет его долю между тремя живыми: при потере Москвы/СПб — по 26,67% / 36,67% / 36,67%; при потере Новосибирска/Владивостока — 30% / 30% / 40%. После восстановления вес возвращается постепенно, чтобы не вызвать всплеск нагрузки.
- При отказе CDN edge площадка withdraw'ит BGP-анонс Anycast-префикса. Трафик автоматически выбирает один из оставшихся маршрутов. При перекосе CDN-трафика используются BGP communities и AS-path prepend, а также контроль утилизации каналов, cache hit ratio, p95 latency и error rate.
