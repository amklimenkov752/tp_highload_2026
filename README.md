...
Примем допущение, что при просмотре карточки товара загружаются в среднем 4 изображения размером 200 КБ каждое. Предположим, что благодаря кешированию на стороне клиента повторно загружается только 50% изображений.

При пиковом RPS просмотра карточки товара 9167 получим:

Traffic_images = 9167 * 4 * 200 000 * 0,5 * 8 / 10^9 ≈ 29,33 Гбит/с.

Предполагается, что изображения доставляются пользователям через CDN. При дополнительном допущении, что 80% запросов к изображениям обслуживаются из кеша CDN, исходящий трафик хранилища изображений составит:

Traffic_origin = 29,33 * 0,2 ≈ 5,87 Гбит/с.
...
|---|---:|
| API | 3,26 |
| Изображения (CDN → пользователи) | 29,33 |
| **Итого к пользователям** | **32,59** |

Пиковый исходящий трафик системы к пользователям составляет приблизительно **32,59 Гбит/с**. Из них 3,26 Гбит/с приходится на API, а 29,33 Гбит/с — на изображения. Дополнительно оценивается трафик между хранилищем изображений и CDN — 5,87 Гбит/с. Он не прибавляется к итоговому трафику к пользователям во избежание двойного учёта.
